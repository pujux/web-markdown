...
```bash
pnpm --filter @web-markdown/playground-fastify start
```
...
curl -i -H 'Accept: text/markdown' http://localhost:3004/guide

# 2) SEO metadata fallback (OpenGraph + Twitter -> front matter)
curl -i -H 'Accept: text/markdown' http://localhost:3004/seo

# 3) Conversion quality sample (headings/lists/code/tables/images)
curl -i -H 'Accept: text/markdown' http://localhost:3004/table

# 4) q-value rejects markdown (HTML passthrough)
curl -i -H 'Accept: text/markdown;q=0, text/html;q=1' http://localhost:3004/guide

# 5) Excluded path stays HTML
curl -i -H 'Accept: text/markdown' http://localhost:3004/not-markdown

# 6) Non-HTML passthrough (JSON)
curl -i -H 'Accept: text/markdown' http://localhost:3004/json

# 7) Redirect passthrough (3xx)
curl -i -H 'Accept: text/markdown' http://localhost:3004/redirect

# 8) Attachment passthrough
curl -i -H 'Accept: text/markdown' http://localhost:3004/download

# 9) Oversize with passthrough behavior
curl -i -H 'Accept: text/markdown' http://localhost:3004/oversize-passthrough

# 10) Oversize with strict not-acceptable behavior (406)
curl -i -H 'Accept: text/markdown' http://localhost:3004/strict/oversize-406

# 11) Strict route still transforms when small
curl -i -H 'Accept: text/markdown' http://localhost:3004/strict/small

# 12) rewriteLink/rewriteImage hook demo
curl -i -H 'Accept: text/markdown' http://localhost:3004/hooks
```
